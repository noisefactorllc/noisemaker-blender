...
vec3 contourConv(vec2 fragCoord, vec2 texelSize, vec3 centerRGB, float lvl, bool useLuma) {
    vec3 northRGB = nmTex(inputTex, (fragCoord + vec2(0.0,  1.0)) * texelSize).rgb;
    vec3 southRGB = nmTex(inputTex, (fragCoord + vec2(0.0, -1.0)) * texelSize).rgb;
    vec3 eastRGB  = nmTex(inputTex, (fragCoord + vec2( 1.0, 0.0)) * texelSize).rgb;
    vec3 westRGB  = nmTex(inputTex, (fragCoord + vec2(-1.0, 0.0)) * texelSize).rgb;
