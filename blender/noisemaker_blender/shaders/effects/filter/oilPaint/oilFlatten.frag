...
void main() {
    // Integer nm_fragment center: every neighbor offset is an integer, so samples
    ...
    // indexed nm_fragment-local array. On WebGL2/ANGLE such an array spills to
    // memory and every one of the ~113 per-pixel accumulations pays a
    // round-trip; explicit variables stay in registers. Each nm_sample is still
    ...
    for (int y = -sampleLimit; y <= sampleLimit; y++) {
        for (int x = -sampleLimit; x <= sampleLimit; x++) {
            ...
            // per-quadrant tests select the sector, and each nm_sample is added
