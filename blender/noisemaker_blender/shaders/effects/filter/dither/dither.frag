...
// so each nm_fragment re-simulates the raster scan over its containing block of
...
#define FS_BLOCK 4
#define FS_APRON_MIN 4
#define FS_APRON_MAX 11
#define FS_RPAD 2
// error row width: block + max apron + right pad + one left pad cell
#define FS_ERR_W (FS_BLOCK + FS_APRON_MAX + FS_RPAD + 1)
...
vec3 errorDiffusion(vec2 globalCoord, float cellSize, ivec2 texSize) {
    ...
    // every index is a compile-time nm_constant once the inner loop unrolls,
    ...
    for (int r = -FS_APRON_MAX; r <= ly; r++) {
        ...
        if (lastRow) {
            // Incoming error for this nm_fragment's own cell; keep the array
            // read on nm_constant indices so it stays register-resident.
    ...
    // This nm_fragment's own pixel, carrying the diffused error so per-pixel
