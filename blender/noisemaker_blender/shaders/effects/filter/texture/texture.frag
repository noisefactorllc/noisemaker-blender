...
float material_edge_mask(vec2 uv, vec2 pixelStep) {
    float l = dot(nmTex(inputTex, uv - vec2(pixelStep.x, 0.0)).rgb, vec3(0.2126, 0.7152, 0.0722));
    float r = dot(nmTex(inputTex, uv + vec2(pixelStep.x, 0.0)).rgb, vec3(0.2126, 0.7152, 0.0722));
    float d = dot(nmTex(inputTex, uv - vec2(0.0, pixelStep.y)).rgb, vec3(0.2126, 0.7152, 0.0722));
    float u = dot(nmTex(inputTex, uv + vec2(0.0, pixelStep.y)).rgb, vec3(0.2126, 0.7152, 0.0722));
...
void main() {
    ...
    float r = shape_material(material_value(globalPixel, globalDims, (gl_FragCoord.xy / vec2(textureSize(inputTex, 0))), materialMotion, 0x1234abcdu));
    vec3 material = vec3(r);
    if (!mono) {
        material.g = shape_material(material_value(globalPixel, globalDims, (gl_FragCoord.xy / vec2(textureSize(inputTex, 0))), materialMotion, 0x68bc21ebu));
        material.b = shape_material(material_value(globalPixel, globalDims, (gl_FragCoord.xy / vec2(textureSize(inputTex, 0))), materialMotion, 0x02e5be93u));
    }
