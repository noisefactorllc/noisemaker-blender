 ...
 *                 representative source nm_sample (see the mosaic branch
 *                 below), and tiles
 ...
 * at runtime on a value that is nm_constant for the whole draw.
 *
...
void main() {
    ...
    // Pixelize the source: every nm_fragment assigned to this warped tile
    // samples one representative coordinate. Evaluate the inverse-warp
    // approximation at the cell center (not at the current nm_fragment), so
    // the nm_sample stays nm_constant across the entire tile interior.
