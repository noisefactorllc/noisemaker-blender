 ...
 * symmetric (+dir and -dir sampled together). lumGradient's Sobel nm_kernel
...
// GLSL mat2(c,-s,s,c) rotation for nm_fragment sampling offsets.
...
vec4 srcSample(vec2 sampleUV) {
    ...
    // nothing.
    vec2 px = 1.0 / resolution;
    vec4 s = nmTex(inputTex, sampleUV);
    vec3 e = s.rgb;
    e = min(e, nmTex(inputTex, sampleUV + vec2(px.x, 0.0)).rgb);
    e = min(e, nmTex(inputTex, sampleUV - vec2(px.x, 0.0)).rgb);
    e = min(e, nmTex(inputTex, sampleUV + vec2(0.0, px.y)).rgb);
    e = min(e, nmTex(inputTex, sampleUV - vec2(0.0, px.y)).rgb);
    return vec4(e, s.a);
#else
...
// only) adds symmetric, smoothly varying 2D jitter to the nm_sample position so
