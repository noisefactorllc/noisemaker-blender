...
// Sobel gradient gradient, applied to the SIMPLIFIED nmTex(pigment pooling edges).
vec2 lumGradientSimplified(vec2 uv) {
    vec2 px = 1.0 / resolution;
    float tl = lum(nmTex(simplifiedTex, uv + px * vec2(-1.0,  1.0)).rgb);
    float  l = lum(nmTex(simplifiedTex, uv + px * vec2(-1.0,  0.0)).rgb);
    float bl = lum(nmTex(simplifiedTex, uv + px * vec2(-1.0, -1.0)).rgb);
    float tr = lum(nmTex(simplifiedTex, uv + px * vec2( 1.0,  1.0)).rgb);
    float  r = lum(nmTex(simplifiedTex, uv + px * vec2( 1.0,  0.0)).rgb);
    float br = lum(nmTex(simplifiedTex, uv + px * vec2( 1.0, -1.0)).rgb);
    float  t = lum(nmTex(simplifiedTex, uv + px * vec2( 0.0,  1.0)).rgb);
    float  b = lum(nmTex(simplifiedTex, uv + px * vec2( 0.0, -1.0)).rgb);
...
void main() {
    ...
    vec3 simplified = nmTex(simplifiedTex, uv).rgb;
